...
// notifyAllWarehouses), автогенерации документов (profileName, autoGenerateDocs,
// autoIncludeAct, autoIncludeM15, autoIncludeYandex), возвратах на склад
// (defaultWarehouse) и офлайн-мосте Яндекс.Диска (yandexSyncData).
const SERVER_PROFILE_KEYS = [
    'profileName', 'defaultWarehouse', 'myTaskKeywords', 'notifyOnlyMine',
    'notifyAllWarehouses', 'autoGenerateDocs', 'autoIncludeAct',
    'autoIncludeM15', 'autoIncludeYandex', 'yandexSyncData'
];
...
function toggleAutoGenerateDocs() {
    const el = document.getElementById('autoGenerateDocsToggle');
    const on = !el.classList.contains('active');
    el.classList.toggle('active', on);
    el.setAttribute('aria-checked', on);
    const sub = document.getElementById('autoGenerateDocsSub');
    if (sub) sub.classList.toggle('d-none', !on);
    saveProfile();
}

function toggleYandexSyncData() {
    const el = document.getElementById('yandexSyncDataToggle');
    const on = !el.classList.contains('active');
    el.classList.toggle('active', on);
    el.setAttribute('aria-checked', on);
...
function openSettings(firstLogin) {
    ...
    const autoGenOn = lsGet('autoGenerateDocs', '') === 'true';
    const autoGenEl = document.getElementById('autoGenerateDocsToggle');
    if (autoGenEl) {
        autoGenEl.classList.toggle('active', autoGenOn);
        autoGenEl.setAttribute('aria-checked', autoGenOn);
        const sub = document.getElementById('autoGenerateDocsSub');
        if (sub) sub.classList.toggle('d-none', !autoGenOn);
    }
    const autoActOn = lsGet('autoIncludeAct', 'true') === 'true';
    const autoActEl = document.getElementById('autoIncludeAct');
    if (autoActEl) autoActEl.checked = autoActOn;
    const autoM15On = lsGet('autoIncludeM15', 'true') === 'true';
    const autoM15El = document.getElementById('autoIncludeM15');
    if (autoM15El) autoM15El.checked = autoM15On;
...
function saveProfile() {
    ...
    const autoGenEl = document.getElementById('autoGenerateDocsToggle');
    if (autoGenEl) lsSet('autoGenerateDocs', autoGenEl.classList.contains('active') ? 'true' : '');
    const autoActEl = document.getElementById('autoIncludeAct');
    if (autoActEl) lsSet('autoIncludeAct', autoActEl.checked ? 'true' : '');
    const autoM15El = document.getElementById('autoIncludeM15');
    if (autoM15El) lsSet('autoIncludeM15', autoM15El.checked ? 'true' : '');
    const autoYdEl = document.getElementById('autoIncludeYandex');
    if (autoYdEl) lsSet('autoIncludeYandex', autoYdEl.checked ? 'true' : '');
    const ydEl = document.getElementById('yandexSyncDataToggle');
    if (ydEl) lsSet('yandexSyncData', ydEl.classList.contains('active') ? 'true' : '');
    updateProfileAvatar();
    ...
    const prof = {
        ...
        autoGenerateDocs: lsGet('autoGenerateDocs', ''),
    ...
    // «оставить текущее» — иначе частичное сохранение молча сбрасывает выключатель
