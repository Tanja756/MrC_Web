...
«Войти по отпечатку» и т.д. Приложение использует тот же PWA: service worker
активен внутри TWA, данные кешируются для офлайн-просмотра.
...
2. Кеширование: при отсутствии связи показываются последние загруженные данные
   (service worker + локальный кеш запросов).
