...
Веб-интерфейс для работы с заявками 1С. Авторизация через 1С (Basic Auth), данные тянутся из HTTP-API 1С и обогащаются из локальной SQLite: кеш остатков складов, трекинг задач (дата закрытия, назначения), справочник магазинов, поломанные единицы оборудования, пуш-подписки, уведомления. Фоновый воркер периодически опрашивает 1С и проверяет новые задачи для отправки уведомлений.

### Возможности

- **Заявки** — три вкладки (в работе / свободные / закрытые), поиск, сортировка (срок, дата создания, дата закрытия, приоритет), закрепление вверху, массовое назначение, GPS-координаты при закрытии, inline-редактирование даты закрытия (локально, без отправки в 1С), возврат в свободные (из описания заявки)
...
- **Документы** — генерация АВР, Акта ФН и М15 в PDF через LibreOffice с подстановкой данных из заявки; М15 поддерживает до 20 товаров с разбивкой на страницы (прямые) и одной обратной без товаров; при >7 товарах в АВР оборудование заменяется общей фразой
...
| `BACKGROUND_CHECK_INTERVAL` | `600` | Интервал фоновой проверки (сек) |
