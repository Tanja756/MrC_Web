...
def compress_image_bytes(raw_bytes: bytes, filename: str = '') -> bytes:
    ext = filename.rsplit('.', 1)[-1].lower() if '.' in filename else ''
    if ext not in ('jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'):
        return raw_bytes
    try:
        ...
        if save_ext == 'JPG':
            save_ext = 'JPEG'
        img.save(out, format=save_ext, quality=QUALITY)
        saved = len(raw_bytes) - len(out.getvalue())
        logger.info(f"compress_image_bytes: {filename} {img.width}x{img.height} saved {max(0, saved)} bytes")
        return out.getvalue()
    except Exception:
        logger.warning(f"compress_image_bytes failed for {filename}, sending original")
        return raw_bytes
